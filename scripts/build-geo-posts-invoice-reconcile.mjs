// build-geo-posts-invoice-reconcile.mjs
// Reads GEO article markdown files from opc-doc (invoice-reconcile), extracts frontmatter + JSON-LD +
...
const ARTICLES_DIR = 'E:/AgentCPM/07_一人公司出海项目/opc-doc/outputs/07-conversion/articles-invoice-reconcile'
const OUT_TS = 'E:/AgentCPM/07_一人公司出海项目/12_Micro_SaaS出海/invoice-reconcile/data/geoPosts.ts'
// NOTE: Vercel alias is NOT invoice-reconcile.lxsaihub.com — after first deploy, replace both with the
// real alias (e.g. invoice-reconcile-<random>.vercel.app) and re-run this script + redeploy.
const PROD_URL = 'https://invoice-reconcile.lxsaihub.com'
const LIVE_DOMAIN = 'https://invoice-reconcile.lxsaihub.com'
...
function parseFrontmatter(md) {
  // Preferred: Owner line, blank line, --- fenced frontmatter --- (matches invoice-reconcile samples)
...
function parseCta(rest) {
  ...
  if (!line) return ''
  const text = line.replace(/\[WAITLIST_URL\]/g, `<a href="${PROD_URL}" class="underline font-semibold">try InvoiceReconcile</a>`)
  return '<p>' + inline(text) + '</p>'
}

function fixDomain(obj) {
  return JSON.parse(JSON.stringify(obj).replace(/https:\/\/invoice-reconcile\.vercel\.app/g, LIVE_DOMAIN))
}
...
for (const f of files) {
  ...
  const description = (blogPosting.description || '').replace(/https:\/\/invoice-reconcile\.vercel\.app/g, LIVE_DOMAIN)
  posts.push({
    slug: fm.slug,
    ...
    description,
    html: mdToHtml(body).replace(/https:\/\/invoice-reconcile\.vercel\.app/g, LIVE_DOMAIN),
    faq: parseFaq(faq),
...
const ts = `// AUTO-GENERATED by scripts/build-geo-posts-invoice-reconcile.mjs — do not edit by hand.
